...
class Kernel:
    ...
    def runner(self,
        ...
        self.log_dir = log_dir or str(int(self.kernelWallClockStart.timestamp()))
        ...
        for sim in range(num_simulations):
            ...
            while not self.messages.empty() and self.currentTime and self.currentTime <= self.stopTime:
                ...
                if ttl_messages % 100000 == 0:
                    print(
                        f"\n--- Simulation time: {self.fmtTime(self.currentTime)}, "
                ...
                if msg_type is MessageType.WAKEUP:
                    ...
                    if self.agentCurrentTimes[agent] > self.currentTime:
                        ...
                        self.messages.put(
                            (self.agentCurrentTimes[agent], (msg_recipient, msg_type, msg))
                        )
                        log_print(
    ...
    def sendMessage(self, sender=None, recipient=None, msg=None, delay=0):
        ...
        sentTime = self.currentTime + pd.Timedelta(
            self.agentComputationDelays[sender] + self.currentAgentAdditionalDelay + delay
        )
        ...
        if self.agentLatencyModel is not None:
            ...
            log_print(
                "Kernel applied latency {}, accumulated delay {}, "
    ...
    def setWakeup(self, sender=None, requestedTime=None):
        ...
        self.messages.put(
            (requestedTime, (sender, MessageType.WAKEUP, None))
        )
    ...
    def setAgentComputeDelay(self, sender=None, requestedDelay=None):
        ...
        if not isinstance(requestedDelay, int):
            raise ValueError(
    ...
    def delayAgent(self, sender=None, additionalDelay=None):
        ...
        if not isinstance(additionalDelay, int):
            raise ValueError(
