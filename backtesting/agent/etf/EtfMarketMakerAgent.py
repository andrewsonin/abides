...
from backtesting.agent.etf.EtfArbAgent import EtfArbAgent
from backtesting.agent.etf.EtfPrimaryAgent import EtfPrimaryAgent
from backtesting.message.base import Message
from backtesting.order.base import BasketOrder
            ...
            if msg.body['msg'] == 'BASKET_EXECUTED':
                ...
                for i, s in enumerate(self.portfolio):
                    if order.is_buy_order:
                        self.holdings[s] -= order.quantity
                    else:
                        self.holdings[s] += order.quantity
                if order.is_buy_order:
